...
public class ArticleAdminService : IArticleAdminService
{
    ...
    public ArticleAdminService(IArticleRepository repository , IBackgroundJobClient _backgroundJobClient)
    {
        this._repository = repository;
        this._backgroundJobClient = _backgroundJobClient;
    }
    ...
    public async Task<ArticleAdminDetailDto> CreateArticleAsync(CreateArticleDto dto)
    {
        ...
        var article = new Article
        {
            ...
            Translations = dto.Translations.Select(t => new ArticleTranslation
            {
                ...
                Title = t.Title,
                Slug = t.Slug,
                Content = Sanitizer.Sanitize(t.Content), // ← sanitize here
                Excerpt = t.Excerpt,
        ...
        var englishTranslation = article.Translations.Where(t => t.LanguageCode == "en").FirstOrDefault();

        if (article.Status == ArticleStatus.Published && englishTranslation != null)
            _backgroundJobClient.Enqueue<INewsletterService>(service =>
                service.NotifySubscribersAsync(article.Id, englishTranslation.Title, englishTranslation.Slug));
    ...
    public async Task<ArticleAdminDetailDto?> UpdateArticleAsync(UpdateArticleDto dto)
    {
        ...
        article.IsTrending = dto.IsTrending;
    ...
    private static HtmlSanitizer CreateSanitizer()
    {
        ...
        // TinyMCE's default toolbar output needs these allowed beyond the library's conservative defaults
